...
Previous work generally measured the quality of optimization algorithms run on the coreset, which we empircally observed to be poor indicator of coreset quality.
...
Despite movement based methods running on very efficient code, it is necessary to compliment them with rather expensive dimension reduction methods, rending what efficiency they might have over importance sampling somewhat moot.
...
The second point of interest focusses on the performance of StreamKM++. The distortion of this algorithm is significantly better than what one would expect from its theoretical analysis.
...
In addition, StreamKM++ currently weighs each point by the number of points assigned to it. It may also be possible to improve the performance of the algorithm in both theory and practise by using a different weighting scheme. 
