...
\chris{Is $w(\cdot)$ missing in the equation above?}
...
\item Evaluating the quality of a coreset is hard: Given two point sets $A$ and $B$, it is hard to determine the distortion when considering $B$ as a candidate coreset of $A$ with respect to $k$-clustering problems in most metrics. \chris{I do not get this point.} Thus, while we can default to the worst case guarantees from theory, it is difficult to compare the output of two coreset algorithms for a given data set. 
\end{itemize}
...
\chris{The sentence is a bit convoluted, especially after `inasmuch as'.}
Nevertheless, this method of comparison has the drawback that it is more likely to measure the performance of the underlying optimization problem, rather than evaluating coresets. Moreover, coresets as defined in \cref{eq:coreset} have a guarantee for \emph{all} candidate solutions $C$ \chris{Do you mean centers here?}. A relaxed guarantee that only preserves optimal or nearly optimal clusterings are known as weak coresets, which often are easier to compute. Thus, if a candidate coreset algorithm outputs a weak coreset, it may have a bad worst-case distortion which would not be visible when evaluating it on the outcome of an optimization algorithm.
