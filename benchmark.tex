...
We now describe how we use the benchmark to measure the distortion of a coreset. Assume that the coresets are subsets of the original input points\footnote{It is not necessary for coreset constructions in general to consists of input points. One can adjust the evaluation to account for this by snapping the coreset points to the closest input point.}.
...
For every cluster $C_i^a$ with $w(C_i^a \cap \Omega)\geq |C_i| (1-\varepsilon)$, we place a center at $\mu(C_i^a)$. Conversely, if $w(C_i^a \cap \Omega)< |C_i^a| (1-\varepsilon)$, we do not place a center at $\mu(C_i^a)$. We call such clusters \emph{deficient}. Let $\calS$ be the centers of the these deficient clusters.
...
Thus, the cost is 
\begin{eqnarray*}
\cost_{C_i^a}(\calS) &\approx & \left(1+\frac{2}{\alpha}\right)\cdot \cost_{C_j^a}(\{\mu(C_j^a)\}).
\end{eqnarray*}
