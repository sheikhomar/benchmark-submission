...
Denote by $\one_k$ 
the $k$-dimensional all-one vector and by $v_i^1$ the $k$ dimensional vector with entries $(v_i^1)_j = \begin{cases}-\frac{1}{k} & \text{if } i\neq j\\
...
To get a better feel for the construction, we have given two small example benchmarks for $k=2$ and $k=3$ in the appendix (see \cref{fig:benchmark-small-instances}).
