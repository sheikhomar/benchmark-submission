...
In addition, we preprocessed the data using the dimension reduction techniques described in Section~\ref{sec:dim_reduction}.
...
We summarized the distortions in \cref{fig:distortions}, for numerical variance we refer to \cref{tab:distortions-mean-std-without-pca} and \cref{tab:distortions-mean-std-with-pca} in the appendix.
...
While all five algorithms are equally matched when optimizing on the candidate coresets, coreset quality performance differ significantly (see \cref{fig:distortions}). We omit tables detailing the cost due to space restrictions. For all data sets, the obtained costs differed insignificantly for all values of $k$ (see \cref{fig:real-costs}).
